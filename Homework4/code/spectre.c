...
void attack(size_t malicious_x, uint8_t value[2], int score[2]) {
  ...
  // 1.- Training the local branch predictor

  // Create a dummy global branch history
  for (int i = 0; i <=4096; i++)
  for (int j = 0; j < 4096; j++)
    temp ^= i;

  // Invoke victim function with valid values
  ...
  // 1.-(b) Training the local branch predictor
